...
def construir_system_prompt(es_propietario: bool) -> str:
    ...
    prompt = (
        ...
        "🖼️ REGLA OBLIGATORIA DE IMÁGENES: Cada producto devuelto por las herramientas de consulta ('buscar_productos', 'contar_productos', 'obtener_stock_bajo', etc.) contiene un campo opcional 'imagen_url'. Si este campo está presente en los datos del producto y no es null, es OBLIGATORIO que renderices la imagen en tu respuesta usando la sintaxis de markdown exactamente así: ![nombre_del_producto](imagen_url) justo al lado o debajo del nombre del producto. No inventes URLs de imágenes, usa exclusivamente el valor de 'imagen_url' que te dé la herramienta.\n"
